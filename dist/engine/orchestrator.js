...
export class AnalysisOrchestrator {
    ...
    cachePath = null;
    fileHashes = new Map();
    constructor(lspServerCommand, lspServerArgs, dbPath, logger) {
    ...
    async linkSymbols(rootDir) {
        ...
        this.store.runInTransaction(() => {
            for (const xmlNode of xmlSqlNodes) {
                ...
                if (mapperInterface) {
                    const mapperMethod = allNodes.find(n => n.type === 'method' && n.file === mapperInterface.file && n.name === xmlNode.name);
                    if (mapperMethod) {
        ...
        this.store.runInTransaction(() => {
            for (const callNode of potentialCalls) {
                const caller = allNodes.find(n => (n.type === 'method' || n.type === 'function') &&
                ...
                if (!caller)
                    continue;
                const targets = this.store.getNodesByName(callNode.name).filter(t => (t.type === 'method' || t.type === 'function' || t.type === 'interface') && t.id !== caller.id);
        ...
        const frontendApiNodes = allNodes.filter(n => {
            try {
                return JSON.parse(n.metadata || '{}').isFrontendApi === true;
            }
            catch {
                return false;
            }
        ...
        const backendEndpointNodes = allNodes.filter(n => {
            try {
                return JSON.parse(n.metadata || '{}').isRestEndpoint === true;
            }
            catch {
                return false;
            }
    ...
    collectFiles(dir, fileList) {
        try {
            ...
            for (const file of files) {
                ...
                if (stat.isDirectory()) {
                    if (file === 'node_modules' || file === '.git' || file === 'dist' || file === 'target' || file === '.guardian')
                        continue;
                ...
                try {
                    ...
                    if (refs) {
                        for (const ref of refs) {
                            ...
                            const callerNode = refNodes.find(n => n.startLine <= ref.range.start.line + 1 && n.endLine >= ref.range.end.line + 1);
                            if (callerNode)
                                edgesToInsert.push({ sourceId: callerNode.id, targetId: nodeId });
                        }
                    }
                }
                catch (err) { }
    ...
    async linkFileSymbols(filePath) {
        ...
        this.store.runInTransaction(() => {
            for (const node of nodes) {
                ...
                if (node.type === 'function' || node.type === 'method' || node.type === 'ref' || meta.isCall) {
                    // 查找匹配的方法或函数定义
                    const targets = allNodes.filter(t => t.name === node.name &&
                        t.id !== node.id &&
