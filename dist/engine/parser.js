...
export class CodeParser {
    ...
    parse(sourceCode, extension) {
        this.setLanguageByExtension(extension);
        return this.parser.parse(sourceCode);
    }
    extractXmlSymbols(content) {
        const symbols = [];
        // 1. Find namespace
    ...
    extractJavaSymbols(content) {
        ...
        if (classNode) {
            ...
            if (requestMapping && requestMapping[1])
                classPath = requestMapping[1];
        }
    ...
    regexExtractJavaSymbols(content) {
        const symbols = [];
        ...
        while ((match = classRegex.exec(content)) !== null) {
            ...
            const name = match[2];
            if (typeStr && name) {
                const type = (typeStr === 'enum' || typeStr === 'record' ? 'class' : typeStr);
    ...
    traverse(node, symbols, extension, context) {
        if (extension === '.java') {
    ...
    traverseTypeScript(node, symbols, context) {
        switch (node.type) {
            case 'function_declaration':
            case 'generator_function_declaration': {
                const fNode = node.childForFieldName('name');
                if (fNode)
                    symbols.push(this.createSymbol(fNode.text, 'function', node));
                break;
            }
            case 'class_declaration': {
                const cNode = node.childForFieldName('name');
                if (cNode)
                    symbols.push(this.createSymbol(cNode.text, 'class', node));
                break;
            }
            case 'interface_declaration': {
                const iNode = node.childForFieldName('name');
                if (iNode)
                    symbols.push(this.createSymbol(iNode.text, 'interface', node));
                break;
            }
            case 'method_definition': {
                const mNode = node.childForFieldName('name');
                if (mNode)
                    symbols.push(this.createSymbol(mNode.text, 'method', node));
                break;
            }
    ...
    traverseJava(node, symbols, context) {
        ...
        switch (node.type) {
            ...
            case 'method_invocation': {
                const nameNode = node.childForFieldName('name');
                if (nameNode)
                    symbols.push(this.createSymbol(nameNode.text, 'variable', node, { isCall: true }));
                break;
            }
    ...
    extractSymbols(content, ext) {
        ...
        if (ext === '.xml')
            return this.extractXmlSymbols(content);
        if (ext === '.vue')
            return this.extractVueSymbols(content);
        return [];
    }
    extractVueSymbols(content) {
        const symbols = [];
    ...
    createSymbol(name, type, node, metadata) {
        return {
            name,
            type,
